...
const SearchResults = () => {
  ...
  const { loading } = useFetch(`search/?q=${query}`);
  ...
  return (
    <SearchVedios>
      <div className="left-bar">
        <div className="flex xl:hidden">{!menu && <LeftBar />}</div>
        <div className="hidden xl:flex">{menu && <LeftBar />}</div>
        {!menu && (
      ...
      <div className="search-results">
        {!loading &&
          url?.contents?.map((item, id) => {
            return <SearchVideoCard video={item.video} key={id} />;
          })}
      </div>
    </SearchVedios>
  );
